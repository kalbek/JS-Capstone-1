class Shows {
    ...
    static getLikesCount = async (itemId) => {
    ...
    static updateUI = async () => {
      ...
      shows.forEach(async (show, index) => {
        const likesCount = await Shows.getLikesCount(show.id); // Call getLikesCount() with the itemId (show.id) to retrieve likes count
        card.innerHTML += `
          <div class="container flex-column">
            ...
            <div class="title-like">
              ...
              <div class="likes">
                ...
                <div class="likes-count"><p>${likesCount} likes</p></div> <!-- Update the likes count here -->
              </div>
            </div>
        ...
        document
          ...
          .addEventListener("click", () => {
            // Handle comments button click event
          });
      });
  
      shows.forEach(async (show, index) => {
        // Add click event listener for likes
        document
          .getElementById(`heart${show.id}`)
          .addEventListener("click", async () => {
            console.log("heart is: ", document.getElementById(`heart${show.id}`));
            await Shows.addLike(show.id); // Call addLike() to add a like for the item
            const updatedLikesCount = await Shows.getLikesCount(show.id); // Call getLikesCount() again to retrieve updated likes count
            document.getElementById(`heart${show.id}`).classList.add("liked"); // Update UI to indicate that the item is liked
            document.getElementById(`heart${show.id}`).innerHTML = "Liked"; // Update UI to show updated like status
            document
              .getElementById(`heart${show.id}`)
              .setAttribute("disabled", "true"); // Disable the like button
            document.getElementById(
              `likes-count-${show.id}`
            ).innerHTML = `${updatedLikesCount} likes`; // Update the likes count in the UI
          });
      });
  
      document.addEventListener("DOMContentLoaded", function () {
        // Select the likeHeart element
